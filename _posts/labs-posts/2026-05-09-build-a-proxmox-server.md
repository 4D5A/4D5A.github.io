...
Linux Containers or LXCs are another virtualization option. LXCs are typically used to virtualize one or more applications without needing to install a dedicated operating system. Since proxmox is built on the linux kernel, you can create LXCs that will run linux avoiding the need for you to create a virtual machine and install a full linux operating system inside of it. You can install and configure applications in your LXCs, and set each LXC to emulate a different linux distribution through the use of proxmox templates, while keeping each LXCs "OS" and application configurations seperate. 
